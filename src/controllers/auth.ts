...
export const AuthController = {
	LOGIN: async (req: Request, res: Response) => {
		...
		try {
			...
			if (!email || !password)
				throw new ErrorException("Username or password is required.");

			connection.beginTransaction();
			const loginResponse: object | IError | null = await AUTH_QUERIES.LOGIN(
				connection,
				{ email, password }
			);
			connection.commit();
	...
	REFRESH_TOKEN: async (req: Request, res: Response) => {
		...
		try {
			const { token }: IRequestFreshToken = req.body;

			if (!token)
				throw new ErrorException("Email and refresh token is required.");
			...
			await connection.query(PRESET_QUERIES.SET_REFRESH_TOKEN, [
				refreshToken,
				email,
			]);
